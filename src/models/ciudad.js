...
class Ciudad {
  ...
  static async getByDeportista(deportistaId) {
    ...
    try {
      const result = await session.run(
        `MATCH (d:Deportista)-[:NACE_EN]->(ciudad:Ciudad)
                 WHERE id(d) = $deportistaId
                 RETURN ciudad`,
        { deportistaId }
      );
      const ciudad = result.records[0]?.get("ciudad");
      return ciudad ? ciudad.properties : null;
    } finally {
      await session.close();
    }
  }
  ...
  static async delete(nombre) {
    const session = driver.session();
    try {
      await session.run(`MATCH (c:Ciudad {nombre: $nombre}) DETACH DELETE c`, {
        nombre,
      });
    } finally {
      await session.close();
