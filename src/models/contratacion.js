...
class Contratacion {
  ...
  static async getByDeportista(deportistaId) {
    ...
    try {
      const result = await session.run(
        `MATCH (d:Deportista)-[:TIENE_CONTRATO]->(contrato:Contrato)-[:CONTRATO_CON]->(equipo:Equipo)
                 WHERE id(d) = $deportistaId
                 RETURN contrato, equipo, date(contrato.fecha_fin) >= date() as activo`,
        { deportistaId }
      );

      const contratos = result.records.map((record) => ({
        contrato: record.get("contrato").properties,
        equipo: record.get("equipo").properties,
        activo: record.get("activo"),
      }));

      return {
        activos: contratos.filter((c) => c.activo),
        antiguos: contratos.filter((c) => !c.activo),
      };
    } finally {
      await session.close();
    }
