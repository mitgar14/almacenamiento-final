...
const noExistenDeportistasPorEquipo = async (id) => {
  ...
  try {
    ...
    if (count > 0) {
      throw new Error(
        `No se puede eliminar el equipo con ID ${id} porque tiene ${count} deportista(s) asociado(s)`
      );
    }
  } finally {
    await session.close();
...
const noExistenContratacionesPorEquipo = async (id) => {
  ...
  try {
    ...
    if (count > 0) {
      throw new Error(
        `No se puede eliminar el equipo con ID ${id} porque tiene ${count} contratacion(es) asociada(s)`
      );
    }
  } finally {
    await session.close();
