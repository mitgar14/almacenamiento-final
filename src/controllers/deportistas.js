...
const obtenerDeportistasPorEquipo = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Error al obtener los deportistas del equipo",
...
const crearDeportista = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al crear el deportista", detalle: error.message });
  }
};
...
const actualizarDeportista = async (req, res) => {
  ...
  try {
    ...
    const deportistaActualizado = await Deportista.update(
      id,
      setClause,
      parametros
    );
    ...
    res
      .status(500)
      .json({
        error: "Error al actualizar el deportista",
        detalle: error.message,
      });
  }
};
...
const eliminarDeportista = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Error al eliminar el deportista",
        detalle: error.message,
      });
  }
};
